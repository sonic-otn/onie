# OTN-KVM x86_64 4-ASIC Virtual Machine
...
#
# The SONiC otn-kvm image is built secure_boot=no, so its ONIE installer takes
# the regular UEFI grub path ("grub-install --target=x86_64-efi") which needs
# the loose x86_64-efi grub modules present in the ONIE runtime. Disabling
# secure boot (and its shim/signing machinery) keeps that path simple and
# avoids requiring per-machine signing keys (make signing-keys-generate).
# This matches upstream's UEFI-capable /onie/efi/ kvm recovery image, which
# ships no shim. OVMF in scripts/build_kvm_image.sh does not enforce secure
# boot (default OVMF_VARS, no PK enrolled), so an unsigned grubx64.efi boots
# fine.
#
# NOTE: all three of SECURE_BOOT_ENABLE, SECURE_BOOT_EXT and SECURE_GRUB must
# be "no" -- the conditionals at the bottom of this file re-enable secure boot
# if any of the higher-level knobs (SECURE_GRUB -> SECURE_BOOT_EXT ->
# SECURE_BOOT_ENABLE) is left on. The kernel config has also been switched to
# the config-insecure variant (no module signing / trusted keyring).
